...
```javascript
{
  ...
  "year": 2022,
  "theme": "철학",
  "keywords": ["Simondon", "Invention", "Technology"],
  "citations": 5,
  "journal": "현대철학"
}
```

### 데이터 정제 파이프라인 (Processing Pipeline)
- **추출 스크립트:** `extract_data_v3.py` (Binary XLS -> JSON 변환 핵심 로직)
- **정제 라이브러리:** Python `xlrd` (바이너리 해독), `pandas` (데이터 프레임 처리).
...
5. **Co-occurrence Network (D3.js v7):** 키워드 간 상관관계. **Label Propagation** 알고리즘으로 Modularity(군집)를 계산하고, Degree Centrality로 노드 크기를 결정함.
6. **Topic Streamgraph (D3.js v7):** 5대 주제의 시계열적 흐름. **Peak-Alignment** 로직을 통해 각 스트림의 가장 두꺼운 지점에 라벨을 배치함.
7. **Citation Map (D3.js v7):** 저자-학술지-담론 간의 네트워크 지형도.

### 기술적 제약 및 해결 (Workarounds)
- **Git:** 시스템 `git` 부재로 인해 `isomorphic-git` 라이브러리를 사용함. 
  *   *Commit Recipe:* `node -e "const git = require('isomorphic-git'); const fs = require('fs'); git.add({fs, dir: '.', filepath: '...'}).then(() => git.commit({...}))"`
